...
class convert_to_cyclic(BaseEstimator, TransformerMixin):
    ...
    def transform(self,X,y=None):
        ...
        for col in X.columns:
            ...
            X[varname]=np.where(pd.isnull(X[col]),1,0)
            newCol.append(varname)
...
class convert_to_cyclic1(BaseEstimator, TransformerMixin):
    ...
    def transform(self,X,y=None):
        newColumn = []
        k=X['Issue'].value_counts()
        for val in k.axes[0][0:10]:
            print(val)
            varname='Issue_'+val.replace(',','_').replace(' ','_')
            X[varname]=np.where(X['Issue']==val,1,0)
            newColumn.append(varname)
...
class convert_to_cyclic2(BaseEstimator, TransformerMixin):
    ...
    def transform(self,x,y=None):
        ...
        k=x['State'].value_counts()[['CA','FL','TX','NY','GA','NJ','PA','IL','VA','MD','OH','NC','MI','AZ','WA']]
        newColumn = []
        for val in k.axes[0][0:15]:
            print(val)
            varname='State_'+val.replace(',','_').replace(' ','_')
            x[varname] =np.where(x['State']==val,1,0)
            newColumn.append(varname)
...
class get_dummies_Pipe1(BaseEstimator, TransformerMixin):
    ...
    def transform(self,x,y=None):
        newColumn=[]
        for col in x.columns:
            temp=pd.get_dummies(x[col],prefix=col,drop_first=True)
            x=pd.concat([temp,x],1)
            newColumn.append(x)
